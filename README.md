...
login or register functionality

```js
import { useState, useEffect } from 'react';
import { Logo } from '../components';
import Wrapper from '../assets/wrappers/RegisterPage';
// redux toolkit and useNavigate later -> we'll set up some functions that asynchronously connect to the API and try to submit the data
...
function Register() {
  ...
  const handleChange = (e) => {
    // as we type, we want to change the values on the state (email & password)
...
```js
// for passing the values as props
// instead of hard-coding each and every time
const FormRow = ({ type, name, value, handleChange, labelText }) => {
  return (
    <div className='form-row'>
      ...
      <input
        type={type} // instead of type='text' -> type = prop!
        value={value}
        name={name}
        onChange={handleChange}
        className='form-input'
...
return (
  <Wrapper>
    ...
    {!values.isMember && ( // only if this is FALSE -> only then display the input!
      <FormRow
        type='text'
        name='name'
        value={values.name}
...
- every time I type something in input
- I also want to change the state value
- we have also to deal with empty values in the submit one
...
const handleChange = (e) => {
  ...
  setValues({ ...values, [name]: value }); // I want to change the property on name dynamically (this is not name, it can be email, password, or whatever) -> we use square brackets passing the variable name and we need to set up the value
  // ...values -> to copy the current state
};
...
- to display some sort of message to the user
- alert message
...
- placeholders: registerUser & loginUser
- both will communicate with our backend
...
const onSubmit = (e) => {
    ...
    if (isMember) {
      // if the user is a member -> dispatch loginUser
      dispatch(loginUser({ email: email, password: password }));
      return;
    }
    // if it is not a member! -> dispatch registerUser
...
- in our case -> options : will be the headers where we're going to place our JASON web token (because on the server, the access is restricted)
- in order to acquire JSON web token in our application, the user must successfully log in OR register

```js
// GET
//axios(url, options); // by default get method!
...
- for our application to work, we need some kind of backend, some kind of server
- somewhere on the cloud, where we can store all of this information
- where we can store users, jobs, modify profiles of the user (WE WILL NOT DO THIS IN THE LOCAL STORAGE)
- We'll save the user in the local storage, but we will not save all of this data in the local storage
- the instructor created this server (check the NODE COURSE of @JohnSmilga)

- Root URL -> where all the requests are going to start
- https://jobify-prod.herokuapp.com/api/v1/toolkit

- Root URL -> where all our requests are going to start and then one by one will add specific endpoint
...
- and we'll use the token in the requests
- the data is protected, only john or whatever user can take a look at the data
- the server is going to be looking for a token that will be getting back initially, whether that is a register user OR login user

- EXAMPLE: complete url for this specific end point:
  https://jobify-prod.herokuapp.com/api/v1/toolkit/auth/register
...
- {name:'john',email:'john@gmail.com',password:'secret'}
- sends back the user object with token
- for register -> we have to provide A UNIQUE EMAIL
- this testing will be just for dummy data!
...
const customFetch = axios.create({
  baseURL: 'https://jobify-prod.herokuapp.com/api/v1/toolkit', // the root url
});
...
export const registerUser = createAsyncThunk(
  ...
  async (user, thunkAPI) => {
    try {
      ...
      console.log(error.response); // to see what we get back from the server
    }
  }
);
```
...
export const registerUser = createAsyncThunk(
  ...
  async (user, thunkAPI) => {
    try {
      ...
      return resp.data; // return if we are successful -> where the user object is going to be sitting
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response.data.msg); // error located in msg property
    }
  }

    // exra reducers in the slice
   extraReducers: {
    [registerUser.pending]: (state) => {
      state.isLoading = true;
    },
    [registerUser.fulfilled]: (state, { payload }) => {
      // grab the payload and right away destructure
      // grab the user form the payload
      const { user } = payload;
      state.isLoading = false;
      state.user = user;
      toast.success(`Hello There ${user.name}`); // name property -> this is from the API
    },
...
export const getUserFromLocalStorage = () => {
  ...
  const user = result ? JSON.parse(result) : null; // if the user is there, we want to parse the result, if not return null
  return user;
};
```

- invoke getUserFromLocalStorage when app loads (set it equal to user)
- import this in the slice
...
// in to places: register & login, in fulfilled!
[registerUser.fulfilled]: (state, { payload }) => {
      const { user } = payload;
      state.isLoading = false;
      state.user = user;
...
- once I log in, wait 3 seconds and go to dashboard
...
- the moment we log out, we also navigate away from the dashboard!
...
- the moment we restrict the access to SharedLayout, we restrict the access to all the pages for the dashboard
...
return (
  <div className='form-row'>
    ...
    {/*SELECT element*/}
    <select
      name='status' // need to match to what we have on the state -> in this case 'status'
      value={status} // remember, here we have default value!
      onChange={handleJobInput}
      className='form-select'
    >
      {/*
       * we iterate over statusOptions
...
const handleJobInput = (e) => {
  const name = e.target.name;
  const value = e.target.value;
  dispatch(handleChange({ name, value })); // we'll be able to control the state every time we make some changed in our inputs
};
```
...
- POST /jobs
- in this point we'll connect to the server and send a POST request
- { position:'position', company:'company', jobLocation:'location', jobType:'full-time', status:'pending' }
- authorization header : 'Bearer token'
- Bearer token => only the logged user can do this!
...
export const createJob = createAsyncThunk(
  ...
  async (job, thunkAPI) => {
    try {
      // post (complete url, the job, authorization headers)
      ...
      // logout user, in case of 401 error!
      if (error.response.status === 401) {
      thunkAPI.dispatch(logoutUser());
      return thunkAPI.rejectWithValue('Unauthorized! Logging Out...');
    }
...
- PATCH /jobs/jobId -> end point -> PATCH : update existing resource on the server !
- { position:'position', company:'company', jobLocation:'location', jobType:'full-time', status:'pending' }
- authorization header : 'Bearer token'
